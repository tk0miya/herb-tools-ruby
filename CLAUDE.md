...
## RBS Type Definition Files (sig/*.rbs)

**NEVER manually edit or generate `.rbs` files.** Type definition files in `sig/` directories are automatically regenerated by a PostToolUse hook whenever `.rb` files in `lib/` are modified. Any manual edits or `rbs-inline` executions will be blocked by a PreToolUse hook. To update type definitions, only modify inline `@rbs` annotations in Ruby source files.
